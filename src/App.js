...
class App extends Component {
  ...
  deleteItem = id => {
    ...
    const historyItems = history.filter(eachItem => eachItem.id !== id)
  ...
  render() {
    ...
    const result = history.filter(eachItem =>
        ...
        <div className="container">
          {result.length > 0 ? (
            <ul className="history-container">
              {result.map(eachItemHistory => (
                <HistoryItem
                  each={eachItemHistory}
                  key={eachItemHistory.id}
